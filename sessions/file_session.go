...
func (this *FileSessionData) fileName(dir string) string {
	return dir + xcode.DS + "session_" + this.Sid + ".data"
}
...
func (this *FileSessionManager) writeSession(session *FileSessionData) bool {
	...
	file := this.realDir() + xcode.DS + "session_" + session.Sid + ".data"
...
func (this *FileSessionManager) Delete(sid string) bool {
	...
	file := this.realDir() + xcode.DS + "session_" + sid + ".data"
	_, err := os.Stat(file)
	if err != nil {
		return true
	}
...
func (this *FileSessionManager) load() {
	...
	// 加载
	go func() {
		defer glog.Println("load sessions from files", this.realDir())

		matches, err := filepath.Glob(this.realDir() + xcode.DS + "session_*")
		if err != nil {
			glog.Errorf("%s", err.Error())
			return
		}
		...
		for {
			...
			this.sessionMap.Range(func(key, value interface{}) bool {
				...
				if sessionObject.isExpired() {
					...
					_ = os.Remove(sessionObject.fileName(this.realDir()))
				} else {
