...
func TestReader_ReadByte(t *testing.T) {
	reader, err := NewReader(xcode.TmpFile("test.txt"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestReader_Read(t *testing.T) {
	file := NewFile(xcode.TmpFile("test.txt"))
...
func TestReader_ReadJSON(t *testing.T) {
	file := NewFile(xcode.TmpFile("test.json"))
	reader, err := file.Reader()
	if err != nil {
		t.Fatal(err)
	}
...
func TestReader_ReadYAML(t *testing.T) {
	reader, err := NewReader(xcode.ConfigFile("server.conf"))
	if err != nil {
		t.Fatal(err)
	}
